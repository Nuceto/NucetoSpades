...
namespace spades {
	...
	class PreferenceView: spades::ui::UIElement {
		...
		void Render() {
			...
			ConfigItem n_TargetColorRed("n_TargetColorRed", "0");
			ConfigItem n_TargetColorGreen("n_TargetColorGreen", "1");
			ConfigItem n_TargetColorBlue("n_TargetColorBlue", "0");
			ConfigItem n_TargetTransparency("n_TargetTransparency", "1");
			ConfigItem linepos("n_TargetLinesPos", "0");
			ConfigItem lineh("n_TargetLinesHeight", "20");
			ConfigItem targetsize("n_TargetSize", "2");
			ConfigItem n_TargetDot("n_TargetDot", "1");
			ConfigItem n_TargetLines("n_TargetLines", "1");
			ConfigItem n_Target("n_Target", "0");
			
			float x = Manager.Renderer.ScreenWidth / 2;
			float y = Manager.Renderer.ScreenHeight - Manager.Renderer.ScreenHeight * 0.95;
			...
			if(n_TargetDot.IntValue > 0 && n_Target.IntValue > 0){
				r.ColorNP = Vector4(n_TargetColorRed.FloatValue / 255.f, 
				n_TargetColorGreen.FloatValue / 255.f, n_TargetColorBlue.FloatValue / 255.f, n_TargetTransparency.FloatValue);
				r.DrawImage(img, 
				AABB2(x - 2/targetsize.FloatValue, y - (2/ targetsize.FloatValue), 4/ targetsize.FloatValue, 4/ targetsize.FloatValue));
			}
			
			if(n_TargetLines.IntValue > 0 && n_Target.IntValue > 0){
				r.ColorNP = Vector4(n_TargetColorRed.FloatValue / 255.f, 
				n_TargetColorGreen.FloatValue / 255.f, n_TargetColorBlue.FloatValue / 255.f, n_TargetTransparency.FloatValue);
				
				r.DrawImage(img, AABB2(x - (2/ targetsize.FloatValue), y - 
				(7/ targetsize.FloatValue)+linepos.FloatValue, 4/ targetsize.FloatValue, -lineh.FloatValue/ targetsize.FloatValue));
				
				r.DrawImage(img, AABB2(x - (2/ targetsize.FloatValue), y + 
				(7/ targetsize.FloatValue)-linepos.FloatValue, 4/ targetsize.FloatValue, lineh.FloatValue/ targetsize.FloatValue));
			
				r.DrawImage(img, AABB2(x - (6/ targetsize.FloatValue)+linepos.FloatValue, y - 
				(2/ targetsize.FloatValue), -lineh.FloatValue/ targetsize.FloatValue, 4/ targetsize.FloatValue));
				r.DrawImage(img, AABB2(x + (6/ targetsize.FloatValue)-linepos.FloatValue, y - 
				(2/ targetsize.FloatValue), lineh.FloatValue/ targetsize.FloatValue, 4/ targetsize.FloatValue));
			}
			
			Font@ font = this.Font;
			string text = "Target Preview";
			if(n_Target.IntValue > 0){
				font.DrawShadow(text, Vector2(x - 51.f, y - 38.f), 1.f,  Vector4(1.f, 1.f, 1.f, 0.8f), Vector4(0.f, 0.f, 0.f, 0.4f));
			}
